...
## How to: Install Weave GitOps Enterprise
...
Also `GITHUB_TOKEN` must be set as an environment variable in the current shell. It should have permissions to create Pull Requests against the cluster config repo.
:::
...
The repository will contain the configurations and manifests for the management cluster and the clusters that are provisioned by Weave GitOps Enterprise.
Currently, GitHub and Gitlab are supported, and you will need a Personal Access Token from your git provider as a part of this installation.

Here is an example using the GitHub `gh` command line tool:
```bash
...
Run the upgrade command specifying the same `--app-config-repo` used in install and the desired version of Weave GitOps Enterprise.
