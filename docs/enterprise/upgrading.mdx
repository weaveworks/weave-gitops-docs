...
Set the name of your cluster to be the same as your clusters kubeconfig
...
Update the `config.cluster.name` value in the Weave Gitops Enterprise `ConfigMap` in your config repo.
...
After configuring values in the `ConfigMap` you will have to delete the `cluster-service`, `gitops-repo-broker` and `event-writer` pods for the changes to take effect.
